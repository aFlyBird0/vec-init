...
func main() {
	...
	for i, p := range processors {
		...
		vectorAndPatentFlow := flows[i].
			...
			Via(flatten[*streamUtil.VectorPatent](20)). // 扁平化
...
func strToVecBatch(p processor.Processor) streams.Flow {
	f := func(patents []*model.Patent) (ms []*streamUtil.VectorPatent) {
		now := time.Now()
		hashID := getHashValue(now.Format("2006-01-02 15:04:05") + p.Field())
		fmt.Printf("%v, 转换专利到向量，本批专利数量：%d, 向量化字段：%v，本次任务标识：%v\n",
			now.Format("2006-01-02 15:04:05"), len(patents), p.Field(), hashID)

		vectors := p.ToVecs(patents)

		for i, patent := range patents {
		...
		fmt.Printf("%v, 转换专利到向量，本批专利数量：%d, 向量化字段：%v, 本次任务标识：%v，转换完成\n",
			time.Now().Format("2006-01-02 15:04:05"), len(patents), p.Field(), hashID)

		return ms
	}

	// 这里并发就设置成1了，上游不支持并行，一并行就出错
	// 后面又设置成2了，因为上游改成了支持多卡
	return flow.NewMap[[]*model.Patent, []*streamUtil.VectorPatent](f, 2)
}

func flatten[T any](parallelism uint) streams.Flow {
	return flow.NewFlatMap(func(element []T) []T {
		return element
	}, parallelism)
}
